...
def print_selected_chimp_info(elements, vcf_index, min_val, miss, out_f):
    chr = elements[0]
    position = elements[1]
    ID = elements[2]
    REF = elements[3]
    ...
    print("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(chr, position, ID, REF, ALT, GT, min_val, miss), file=out_f)
...
with gzip.open(vcf_input_file, "rt") as f, \
    ...
    print("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("chr", "position", "ID", "REF", "ALT"," GT", "min_count", "missingness"), file=out_fh)
    for line in f:
        ...
        if line.startswith("#CHROM"):
            ...
            if all(value in fixed1 for key, value in Pantro.items()) or \
               ...
               any(value.split("/")[1] in str(range(2, 9)) for key, value in Pantro.items()):
                ...
                print_selected_chimp_info(fields, analized_vcf_index, minim, missingness, out_fh)
