...
def take_info_from_beagle_vcf_file(line1, missings_dict, imputed_dict, index2):
    fields = line1.rstrip().split()
    position = fields[1]
    allele0 = fields[3]
    allele1 = fields[4]
    ...
    if r_square >= 0.3:
        if position in missings_dict and \
           (allele0 in missings_dict[position] and allele1 in missings_dict[position]):
            ...
            imputed_dict[position] = [genotype, r_square]
    return imputed_dict
...
with gzip.open(vcf_out_file, "rt") as f2:
        for line in f2:
            ...
            if not line.startswith("#"):
                imputed_gens = take_info_from_beagle_vcf_file(line, missings, imputed_gens, sample_index2)
