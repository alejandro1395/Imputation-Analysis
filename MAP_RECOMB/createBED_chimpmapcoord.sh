...
out=${OUTDIR}"out/"
qu=${OUTDIR}"qu/"
jobname=${qu}"createBED_chimpmapcoord"
...
echo ${chromosomes} | tr " " "\n" | while read chr;
do echo "#!/bin/bash
python ${SRC}createBED_chimpmapcoord.py ${IN}chimp_map_chr${chr}.map ${OUTDIR}chimp_map_BED_chr${chr}.bed" > ${jobname}_chr${chr}.sh
chmod 755 ${jobname}_chr${chr}.sh
/scratch/devel/avalenzu/CNAG_interface/submit.py -c ${jobname}_chr${chr}.sh -n createBED_chimpmapcoord -o ${out}BED.out -e ${out}BED.err -u 1 -t 1 -w 01:00:00
done
